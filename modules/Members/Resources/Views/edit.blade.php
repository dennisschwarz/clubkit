...
<div class="max-w-lg space-y-6">
    ...
    <div>
        <h1 class="text-2xl font-bold text-gray-900">{{ $member->last_name }}, {{ $member->first_name }}</h1>
        <p class="text-sm text-gray-500 mt-1">
            <a href="{{ route('members.index') }}" class="text-blue-600 hover:underline">&larr; Zurück zur Liste</a>
        </p>
    </div>

    <form method="POST" action="{{ route('members.update', $member) }}" class="bg-white rounded-xl shadow-sm border border-gray-200 p-6 space-y-5">
        @csrf
        @method('PATCH')
    ...
    <div class="bg-red-50 border border-red-200 rounded-xl p-5">
        <h3 class="text-sm font-bold text-red-700 mb-2">Mitglied löschen</h3>
        <p class="text-xs text-red-600 mb-3">Das Mitglied wird als gelöscht markiert (Soft Delete).</p>
        <form method="POST" action="{{ route('members.destroy', $member) }}"
              onsubmit="return confirm('Mitglied wirklich löschen?')">
            @csrf @method('DELETE')
