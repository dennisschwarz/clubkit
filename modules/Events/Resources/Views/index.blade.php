...
@section('content')
<div class="ck-page-header"> <div> <h1 class="ck-page-title">Termine</h1> <p class="ck-page-subtitle">{{ $events->total() }} Termin{{ $events->total() !== 1 ? 'e' : '' }} gesamt</p> </div> <x-ck-button variant="primary" onclick="evtModalOpen()"> + Termin anlegen </x-ck-button> </div>
@if($events->isEmpty())
<x-ck-card>
...
@else
<div class="ck-table-wrap"> <table class="ck-table"> <thead> <tr> <x-ck-sort-header column="starts_at" label="Datum / Zeit" /> <x-ck-sort-header column="title" label="Titel" /> <x-ck-sort-header column="location" label="Ort" /> {{-- Teams injects the <th> for the teams column here. Extension point: event.table.teams.header Registered by: TeamsServiceProvider Only present when Teams is active. --}} @ckHook('event.table.teams.header') <th>Besetzung</th> @ckHook('event.table.header') <th class="ck-table__actions">Aktionen</th> </tr> </thead> <tbody> @foreach($events as $event) @php $isPast = $event->starts_at->isPast(); @endphp <tr class="{{ $isPast ? 'ck-table__row--muted' : '' }}">
            {{-- Date / time --}}
...
@if($events->hasPages())
<div class="ck-table__pagination ck-table__pagination--standalone">{{ $events->links() }}</div> @endif @endif
{{-- ══ Quick-Create Modal ══════════════════════════════════════════════════════ --}}
...
@push('scripts')
<script> window.CK_Events = { routes: { store: "{{ route('events.store') }}" } }; </script>
