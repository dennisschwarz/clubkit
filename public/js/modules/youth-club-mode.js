...
(function () {
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        var relSelect = document.getElementById('mFieldRelationship');
        if (relSelect) {
            relSelect.addEventListener('change', _onRelationshipChange);
        }

        // Mitglieds-Dropdown: Hinzufügen-Button freischalten
        var memberSelect = document.getElementById('mFieldRelatedMember');
        if (memberSelect) {
            memberSelect.addEventListener('change', function () {
                var addBtn = document.getElementById('mBtnAddRelation');
    ...
    function _onRelationshipChange() {
        ...
        var relationship = relSelect.value;
        _hideAddError();
        ...
        var filtered = _getFilteredMembers(relationship);
    ...
    function _getFilteredMembers(relationshipType) {
        var ycm       = window.CK_YouthClubMode || {};
        var allMem    = ycm.allMembers  || {};
        var relations = ycm.relations   || [];
        var all       = Object.values(allMem);

        // Immer: sich selbst ausschließen
        var result = all.filter(function (m) { return m.id !== currentMemberId; });
    ...
    function _onAddRelation() {
        ...
        var relationship    = relSelect.value;
        var relatedMemberId = memberSelect.value;
        ...
        var ycm = window.CK_YouthClubMode || {};
        var url = ((ycm.routes || {}).relationsBase || '') + '/' + currentMemberId + '/relations';
    ...
    window.ckYcmDeleteRelation = function (relationId) {
        ...
        var ycm = window.CK_YouthClubMode || {};
        var url = ((ycm.routes || {}).relationsBase || '')
                + '/' + currentMemberId
    ...
    function _renderFamilyList() {
        var list = document.getElementById('mFamilyList');
        if (!list || !currentMemberId) return;

        var ycm       = window.CK_YouthClubMode || {};
        var relations = ycm.relations  || [];
        var allMem    = ycm.allMembers || {};
        var family    = _buildFamily(currentMemberId, relations, allMem);
        ...
        family.children.forEach(function (child) {
            var label = child.parent_relation === 'father' ? 'Kind (als Vater)' : 'Kind (als Mutter)';
     ...
     * Spiegelt die Logik aus YouthClubModeServiceProvider::buildFamilyData().
     */
    function _buildFamily(memberId, relations, allMem) {
        var family = { father: null, mother: null, children: [], siblings: [] };

        relations.forEach(function (r) {
            var pid = r.primary_member_id;
            var sid = r.secondary_member_id;
            var rel = r.relationship;
            ...
            if (rel === 'sibling' && (pid === memberId || sid === memberId)) {
                var otherId = pid === memberId ? sid : pid;
    ...
    function _showAddError(msg) {
        var el = document.getElementById('mFamilyAddError');
        if (!el) return;
        el.textContent = msg;
